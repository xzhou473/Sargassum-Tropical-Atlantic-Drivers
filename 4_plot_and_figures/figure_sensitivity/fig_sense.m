...
load('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_sensitivity/yp_control.mat');
...
resultspath1 = '../utils/newdata/fldm_of_1993_2022_glorys_mlt_reducedGASB_masked_deseas_1-12deg.nc' 
...
resultspath3 = '../utils/newdata/fldm_of_duaod550_f64_res075_2003-2022_deseas_reducedGASB.nc' 
...
Window_for_MLT_timeser = lagii%5;%5;     % N-month moving mean. 5
...
%tbl =table(MLT_shifted(Training_ini:Training_end),conc_previous(Training_ini:Training_end),DUST(Training_ini:Training_end),SST_nonlinear(Training_ini:Training_end),MLT_avgnorm(Training_ini:Training_end),remaining_conc(Training_ini:Training_end),conc_current_year(Training_ini:Training_end)); 
tbl =table(MLT_avgnorm0(Training_ini:Training_end),conc_previous(Training_ini:Training_end),DUST(Training_ini:Training_end),SST_nonlinear(Training_ini:Training_end),MLT_avgnorm1(Training_ini:Training_end),remaining_conc(Training_ini:Training_end),conc_current_year(Training_ini:Training_end)); 
...
%yp1 = (b1*exp(MLT_shifted(1:end)).^abs(b2) + (conc_previous.^abs(b3)) + b4*(DUST(1:end))+b5*(SST_nonlinear(1:end))+b6*(remaining_conc(1:end).*exp(b7.*(MLT_avgnorm(1:end)'-b8)))).*sign(heaviside(b1*exp(MLT_shifted(1:end)).^abs(b2) + (conc_previous.^abs(b3)) + b4*(DUST(1:end))+b5*(SST_nonlinear(1:end))+b6*(remaining_conc(1:end).*exp(b7.*(MLT_avgnorm(1:end)'-b8)))));
elseif notemperature == 5
...
legend([h3 h2],'obs (digitized)','model','Location','northwest')
...
yp_all(counter,1)=corr(obs_webdig, yp1);
yp_all(counter,2)=mean((yp1 - obs_webdig).^2);
yp_all(counter,3)=corr(yp_control, yp1);
yp_all(counter,4)=mean((yp1 - yp_control).^2);
