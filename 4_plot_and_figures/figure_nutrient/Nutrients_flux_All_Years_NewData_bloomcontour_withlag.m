...
%% ------------------------------------------------------------------------------
%% This is the same as script BUT 
%% it shifts the MLT timeseries (both the post bloom and the pre-boom climatology)
%% AND the Nutrients timeseries 2 MONTHS in the PAST, BEFORE DOING EVERYTHING
%% ------------------------------------------------------------------------------
...
Ctab = csvread('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/Fig1_d_Bach_et_al_2021_MillionTon.csv');
...
% THEN WE WILL SHIFT IT N MONTHS.

 %ncid = netcdf.open('../utils/nutriflux_new_bloomcontour/MLT_2011-2022_f64_025_masked_bloomcontour.nc')  
 ncid = netcdf.open('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nutrient/nutriflux_new_bloomcontour/nutriflux_new_bloomcontour/MLT_2011-2022_f64_025_masked_bloomcontour.nc')
  ...
  z2_noshift = double(netcdf.getVar(ncid,3));    %3 always check

netcdf.close(ncid);

% At each timestep i want to have the z2 that I had TOT months before.
% Shift is along dimension 3 (time dimension), which needs to be specified
% as we are shifting an array and not a vector. 

LAG_TO_SHIFT = 3 ; % PUT 2 OR 3 DEPENDING ON WHAT YOU GET BY THE REGRESSION SCRIPT
ALONG_DIMENSION = 3;

z2 = circshift(z2_noshift,LAG_TO_SHIFT,ALONG_DIMENSION);

%
% Verify that z2 is z2_noshift 2 months in the past. Cioè quello che ho in
% ogni istante t in z2 deve essere uguale 
% a quello che avevo all'istante passato t-2 in z2_noshift

% Questa diffreenza deve essere zero at each time 

test_difference(1:length(z2(:,1,1)), 1:length(z2(1,:,1)),  1:length(z2(1,1,:))) = NaN; % Initialize dummy 

for t = LAG_TO_SHIFT+1:length(z2(1,1,:)) % Parto da tre perchè sennò i primi due timestep vengno negativi

    test_difference(:,:,t) = (z2(:,:,t) - z2_noshift(:,:,t-LAG_TO_SHIFT)); 

end 

%% Check - The diff must be always zero for the lag to be applied correctly
tdresh = reshape(test_difference,[1,360*81*144]);

difvector = [min(tdresh) max(tdresh)]
difvector = abs(abs(difvector(1)) + abs(difvector(2)))

if difvector > 0 
    error ' the shift of MLT is wrong - please review that part of this script' 
else 
    disp ' shift z2 ok '
end 
...
ncid = netcdf.open('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nutrient/nutriflux_new_bloomcontour/nutriflux_new_bloomcontour/ymonmean_mlt_1999-2010_f64_025_masked_repeat_bloomcontour.nc') 
  ...
  z1_noshift = double(netcdf.getVar(ncid,4));    %3 always check
netcdf.close(ncid);

% At each timestep i want to have the z1 that I had two months before.
% Shift is along dimension 3 (time dimension), which needs to be specified
% as we are shifting an array and not a vector. 


z1 = circshift(z1_noshift,LAG_TO_SHIFT,ALONG_DIMENSION);



% Questa diffreenza deve essere zero at each time 

test_difference(1:length(z1(:,1,1)), 1:length(z1(1,:,1)),  1:length(z1(1,1,:))) = NaN; % Initialize dummy 

for t = LAG_TO_SHIFT+1:length(z1(1,1,:)) % Parto da tre perchè sennò i primi due timestep vengno negativi

    test_difference(:,:,t) = (z1(:,:,t) - z1_noshift(:,:,t-LAG_TO_SHIFT)); 
...
% ogni anno per 12 anni, vedi sh script. 
ncid = netcdf.open('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nutrient/nutriflux_new_bloomcontour/nutriflux_new_bloomcontour/ymonmean_NUTRI_1999-2010_repeat_bloomcontour.nc') % *** LN: QUESTI SONO DA RICONTROLLARE, E CAPIRE SE DEVONO ESSERE DI PERIODO CONSISTENTE CON DATI MLT OPPURE NO...
  lon = double(netcdf.getVar(ncid,2));    % 1 Always cross-check if it's 1
  lat = double(netcdf.getVar(ncid,3));    % 2 Always cross-check if it's 2
  depth_nutri = (netcdf.getVar(ncid,4));  % 3 always check
  no3_noshift = double(netcdf.getVar(ncid,5));    % 4 always check
netcdf.close(ncid);

%%

% At each timestep i want to have the no3 that I had two months before.
% Shift is along dimension 3 (time dimension), which needs to be specified
% as we are shifting an array and not a vector. 

ALONG_DIMENSION_NO3 = 4 ; 

no3 = circshift(no3_noshift,LAG_TO_SHIFT,ALONG_DIMENSION_NO3);

%%
% Questa diffreenza deve essere zero at each time 
clear test_difference
test_difference(1:length(no3(:,1,1,1)), 1:length(no3(1,:,1,1)), 1:length(no3(1,1,:,1)),1:length(no3(1,1,1,:))) = NaN; % Initialize dummy 
%%
for t = LAG_TO_SHIFT+1:length(no3(1,1,1,:)) % Parto da qui perchè sennò i primi tot timestep vengno negativi

    test_difference(:,:,:,t) = (no3(:,:,:,t) - no3_noshift(:,:,:,t-LAG_TO_SHIFT)); 
end 

% Check - The diff must be always zero for the lag to be applied correctly

% timeser nuova = timeservecchia due mesi prima 
% quindi la timeser nuova deve anticipare quella vecchia di due mesi. 


tdresh = reshape(test_difference,[1,360*81*29*144]);

difvector = [min(tdresh) max(tdresh)]
difvector = abs(abs(difvector(1)) + abs(difvector(2)))

if difvector > 0 
    error [' the shift of NO3 is wrong - please review that part of this script'] 
else 
    disp ' shift no3 ok '
end 
...
ncid = netcdf.open('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nutrient/nutriflux_new_bloomcontour/nutriflux_new_bloomcontour/gridcell_area_new_025.nc')
...
ylim([0 0.26])
%print(gcf,'../Figures/NutriFlux_allyears_bloomcontour_lag3months.png','-dpng','-r350')
...
save('N_Vol_in_Mton_box_spacesum_bloomcontour_withlag.mat','N_Vol_in_Mton_box_spacesum')

%% Save the final workspace: 
%save Nutrients_workspace_withlag.mat
