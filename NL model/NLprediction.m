...
legend([h3 h2 h4],'Observations','Model (trained)','Model (predicted)','Location','northwest')
...
outfile=['/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_prediction/Fig5.png'];print([outfile],'-dpng','-r300');
...
for yptest1=1:length(MLT_shifted)
if MLT_shifted(yptest1) >=0
testold1(yptest1) = b1.*(MLT_shifted(yptest1)).^abs(b5); 
testold2(yptest1) = b2*(1-MLT_avgnorm1(yptest1).^3).*conc_previous(yptest1);
testpredict1(yptest1) = b1.*(MLT_shifted(yptest1)).^abs(b5);
testpredict2(yptest1) = b2*(1- MLT_avgnorm_test1(yptest1).^3).*conc_previous(yptest1);
else
testold1(yptest1) = -b1.*(abs(MLT_shifted(yptest1))).^abs(b5) ;   
testold2(yptest1) = b2*(1-MLT_avgnorm1(yptest1).^3).*conc_previous(yptest1) ;
testpredict1(yptest1) = -b1.*(abs(MLT_shifted(yptest1))).^abs(b5); 
testpredict2(yptest1) = b2*(1- MLT_avgnorm_test1(yptest1).^3).*conc_previous(yptest1);
end 
end
...
box on 
set(gca,'GridLineStyle',':')
plot(datevector(a:b-NORMINDEX2),testold1(1:NORMINDEX),'o-','color',[0.01 0.01 0.5],'linewidth',2.0,'MarkerSize',4,'MarkerFaceColor',[0.01 0.01 0.5]);hold on
plot(datevector(a:b-NORMINDEX2),testold2(1:NORMINDEX),'o-','color',[ 0.7461 0.5039 0.1758],'linewidth',2.0,'MarkerSize',4,'MarkerFaceColor',[ 0.7461 0.5039 0.1758]);hold on
plot(datevector(end-NORMINDEX2:end),testpredict1(end-NORMINDEX2:end),'-.o','color',[0.01 0.01 0.5],'linewidth',2.0,'MarkerSize',4,'MarkerFaceColor',[0.01 0.01 0.5]);hold on
plot(datevector(end-NORMINDEX2:end),testpredict2(end-NORMINDEX2:end),'-.o','color',[ 0.7461 0.5039 0.1758],'linewidth',2.0,'MarkerSize',4,'MarkerFaceColor',[ 0.7461 0.5039 0.1758]);hold on
set(gca,'FontSize',15); 
set(gca,'XMinorTick','on')
set(gca,'YMinorTick','on')
set(gca,'TickDir','out')
