...
set(gca,'TickDir','out')
set(gca,'XMinorTick','on','YMinorTick','on')
text(-84.0,4.0,['Mean = ' num2str(nanmean(data1plot(:))) '\circ C'])
text(-84.0,2.0,['STD = 0.3043  \circ C'])
cb = colorbar;
ylabel(cb,'SST anomaly (\circ C)');
%outfile=['/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nonlinearmodel/sigmaissue/SST_sigma1.png'];print([outfile],'-dpng','-r300');
...
set(gca,'fontsize',13)
legend('Deseasonalized data','Deseaonalized and detrend data')
ylabel('SST anomaly (m)')
xlim([2012 2023])
%ylim([-6 8])
text(2019, -0.55,['Mean = ' num2str(nanmean(MLD_detrend)) '\circ C']);
text(2019,-0.75, ['STD = 0.2308  \circ C'])
%outfile=['/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/paperfigure/figure_nonlinearmodel/sigmaissue/SST_sigma2.png'];print([outfile],'-dpng','-r300');

%%%%%73 means start at 1999
...
lgd=legend({['Mean SST anomaly'], ...
        ['1999-2010 mean: ' num2str(mean(MLD_annual_mean(i,1:12),2), '%.2f') ' (\circC)'], ...
