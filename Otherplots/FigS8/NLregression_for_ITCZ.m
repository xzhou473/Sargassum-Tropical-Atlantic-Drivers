...
%resultspath1 = '../utils/newdata/domain/GASB/fldm_of_1993_2022_glorys_mlt_BLOOM-AREA_masked_deseas_1-12deg.nc' 
resultspath1 = '/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/ITCZ/fldm_of_1993_2022_glorys_mlt_reducedGASB_masked_deseas_ITCZ.nc' 
...
%resultspath3 = '../utils/newdata/domain/GASB/fldm_of_duaod550_f64_res075_2003-2022_deseas_BLOOM-AREA.nc' 
resultspath3 = '/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/ITCZ/fldm_of_duaod550_f64_res075_2003-2022_deseas_reducedGASB_ITCZ.nc' 
...
SSTmean=ncread('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/ITCZ/fldm_of_1993_2022_glorys_sst_reducedGASB_masked_deseas_ITCZ.nc','thetao');
%SSTmean=ncread('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/GASB/fldm_of_1993_2022_glorys_sst_BLOOM-AREA_masked_deseas_1-12deg.nc','thetao');
...
%% Plot the resulting biomass to cross check that it makes sense compared to Fig 1(e) of Jouanno et al 2023.
...
%title('Digitized monthly mean Sargassum biomass','Interpreter','latex');
%grid on 
%box on 
...
%h2 = bar(observation_dates,obs_webdig);
%h2.FaceColor = [.0 .0 .0]
%h2.FaceAlpha = 0.4
...
%MLT_shifted = circshift(MLT,MESI_mlt);      % Shifted N months in teh past.  
                                            % At each timestep, given that
                                            % mlt precedes sarg, I need to
                                            % use the mlt that I had tot
                                            % months before. 
                                            %(and then we can do sensitivity to lags)

%xing modified
%%%%normalized MLT first
%MLT=(MLT-min(MLT))./(max(MLT)-min(MLT));
%MLT=(MLT-mean(MLT))./(std(MLT));
...
%%%%%test tropical region here
load('/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/ITCZ/ITCZ_sargassum.mat');
obs_webdig_ITCZ=obs_webdig;
obs_webdig_ITCZ(1:120)=MonthlyMean3;%obs_webdig(1:120).*Sp;%%%ITCZ region only have 10 years (120 data points)
...
%%%%X is just a movmean 3
x=movmean(obs_webdig_ITCZ,3);
if false
for k = 1:(length(x)/3)-1
    observation_dates(3*k:(3*k)+2);
    x(3*k:(3*k)+2) = mean(obs_webdig_ITCZ(3*k:(3*k)+2)); %test I
end
x(1:2) = mean(obs_webdig_ITCZ(1:2));
x(end) = mean(obs_webdig_ITCZ(end));
x = x';
end
...
toty=12;%9;%12; %%9 month lag or 12 month lag
...
Training_end = 120;%length(observation_dates); % ends at the last timestep
clear tbl
...
%yp1 = (b1.*(MLT_avgnorm0(1:end)).^abs(b5) + b2*(1-MLT_avgnorm1(1:end).^3).*conc_previous + b3.*(DUST(1:end))).*sign(heaviside(b1.*(MLT_avgnorm0(1:end)).^abs(b5) + b2*exp(1-MLT_avgnorm1(1:end)).*conc_previous + b3.*(DUST(1:end))));
...
%%%ALL R VALUE
text(2025,23,['R=' sprintf('%.2f', corr(obs_webdig(Training_ini:Training_end), yp1(Training_ini:Training_end)'))],'Fontsize',15)
text(2025, 21, ['MSE = ' sprintf('%.2f', mean((yp1(Training_ini:Training_end)' - obs_webdig(Training_ini:Training_end)).^2))], 'FontSize', 15)
xlim([datevector(a) datevector(b-24)]);
...
%outfile=['/media/xzhou473/Seagate Backup Plus Drive/Sargassum/Sarg_for_Xing/Sarg_for_Xing/utils/newdata/domain/ITCZ/Fig3_9monthlag.png'];print([outfile],'-dpng','-r300');
...
box on 
set(gca,'GridLineStyle',':')
plot(datevector(a:b),test1,'ro-','linewidth',1.5,'MarkerSize',4);hold on
plot(datevector(a:b),test2,'bo-','linewidth',1.5,'MarkerSize',4);hold on
set(gca,'FontSize',15); 
set(gca,'XMinorTick','on')
set(gca,'YMinorTick','on')
set(gca,'TickDir','out')
